...
Modelled on the work item that motivated the change — a title about a payment
...
def issues(db, project):
    ...
    return {
        "gateway": _issue(
            ...
            "Sage USA is the incumbent gateway. Compare SAGE-NUVEI Level 3 rates before the review.",
        ),
...
class TestWorkItemSearchFindsBodies:
    def test_a_word_only_in_the_body_is_found(self, session_client, workspace, issues):
        ...
        results = _search(session_client, workspace.slug, "sage")
    ...
    def test_search_is_case_insensitive_in_the_body(self, session_client, workspace, issues):
        assert len(_search(session_client, workspace.slug, "SAGE")) == 1
...
class TestSequenceIdLookup:
    ...
    def test_identifier_and_number_together_find_it(self, session_client, workspace, issues):
        target = issues["gateway"]
        results = _search(session_client, workspace.slug, f"DUROPC-{target.sequence_id}")
        assert target.name in _names(results)

    def test_a_number_inside_a_phrase_is_not_an_id_lookup(self, session_client, workspace, issues):
        """Prose containing a number must not drag in the record carrying it.

        The decoy is matched by neither word, and its sequence id appearing in
        the query is not a reason to return it.
        """
        decoy = issues["decoy"]
        results = _search(session_client, workspace.slug, f"payment gateway {decoy.sequence_id}")
        assert decoy.name not in _names(results)
...
class TestSearchDoesNotWidenVisibility:
    ...
    def test_another_tenants_work_item_is_not_returned(self, session_client, workspace, issues):
        ...
        _issue(other_project, "Their gateway work", "Sage everywhere in this body too.")
    ...
    def test_a_project_the_caller_left_is_not_searched(self, session_client, workspace, project, issues, create_user):
        ...
        assert _search(session_client, workspace.slug, "sage") == []
